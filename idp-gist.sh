...
# cannot seem to be able to access via the URL below
...
# Name - idp-demo
# Project - production
# Repository URL - value from REPO_URL
# SSH private key

##################################
# Commit changes from setup.sh   #
##################################
...
kubectl create secret generic port-secrets \
  --from-literal=portClientId="${CLIENT_ID}" \
  --from-literal=portClientSecret="${CLIENT_SECRET}" \
  -n argocd

cp argocd/port.yaml infra/.
